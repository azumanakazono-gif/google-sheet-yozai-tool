...
- **「週次ステータス変更履歴」への追記位置が大きくズレる(例: 103行目のはずが655行目に追記される)**: `Sheet.getLastRow()`はシート全体(どの列でもよい)の最終行を返すため、実際のデータとは無関係な列に離れた場所まで書式やゴミデータが残っていると、実データの最終行より大幅に手前/先の行を誤って返すことがある。これを避けるため、`appendStatusHistoryRows_`は`findLastRowWithValueInColumn_`で記録日時列(通常A列)に絞って実際の最終行を探し、その次の行に追記する。
- **「週次ステータス変更履歴」の転記項目が列ごとズレる(例: 案件名がE列に、変更経緯がK列ではなくL列に入る)**: 履歴シートの実際のヘッダー行に空欄の列(削除済み列の跡など)が挟まっていると発生していた。旧`ensureColumnsExist_`はヘッダー行から空欄セルを`.filter()`で詰めてから列名→列番号のマップを作っていたため、空欄列より右側の列名が実際の列位置とズレるだけでなく、空欄がある度に既存のヘッダー行そのものを詰めて上書きする副作用もあった。現在は空欄を詰めずに実際の列位置をそのまま保持するように修正済み(該当PRの`WeeklySync.gs`/`コード.gs`の`ensureColumnsExist_`を参照)。この修正より前の実行で既にヘッダー行が詰められてしまっている場合は、シートの列見出しと実データの対応を目視で確認し、必要なら手動で列を復元すること。
