...
3. 記録日時・案件名(`CONFIG.CONFIDENCE_EDIT_COLUMN_LETTERS['案件名']`列)・変更前後の見込確度・
   変更経緯(自動生成の説明文 + 入力された理由)などを、バッチ取り込み時と同じ
   `STATUS_HISTORY_COLUMNS`の列構成で「週次ステータス変更履歴」の末尾に1行追記する。
   案件名セルのリンク・スタイルや昇降フラグ列の扱いもバッチ取り込み時と同様
   (リンク先はこのシートの編集された行)。
...
- **「今月」シートでの手動編集(`onConfidenceCellEdited`)による転記データが完全にズレる(例: 案件名列に案件名ではなく「[今月 164行目]」が入る、担当・次回アクション・最新見積日・売上金額等が一切反映されない)**: 旧`onConfidenceCellEdited`は「今月」シートの1行目をヘッダー行としてスキャンし、そのヘッダー名を「週次報告記録」側と共通の列名候補(`STATUS_HISTORY_KEY_COLUMN_CANDIDATES`等)でマッチングしていたが、「今月」シートの実際の列見出しはこれらの名前と一致しない(または見出し行の構成自体が異なる)ため、ほぼ全ての列でマッチングに失敗し、案件名は`keyCol`が見つからずフォールバック表示("(今月 164行目)")になり、他の項目も空欄のまま記録されていた。現在はヘッダー名でのマッチングを廃止し、`CONFIG.CONFIDENCE_EDIT_COLUMN_LETTERS`に設定した**固定の列位置**(案件名=C, 担当=D, カテゴリ=E, 次回アクション予定日時=K, ネクストアクション=L, 見込確度=O, 最新見積格納日=R, 契約見込月=S, 税込想定売上=AC など)から`readConfidenceEditField_`で直接値を抽出するように修正済み。「今月」シートの列位置が変わった場合は、この設定を更新すること。
