...
const CONFIG = {
  ...
  // 【重要: 期(会計年度)が変わったときの切り替え方】getTargetSpreadsheet_()(Utils.gs)が
  ...
  // 【重要】appendStatusHistoryRows_(StatusHistory.gs/コード.gs)は、STATUS_HISTORY_COLUMNS
  // のうちこの配列に含まれる列名には一切値を書き込まない(セルに直接触れない)。
  // 「昇降フラグ」はシート側の数式(H列・I列を比較する数式)を保持するため、
  // 「変更経緯」は手動編集時のみwriteChangeReason_で個別に(理由が入力された場合だけ)
  // 追記更新する運用のため、それぞれ除外している。以前は行全体を履歴シートの最終列幅の
  // 配列としてまとめて書き込んでいたため、これらの列も含めて空文字で上書きしてしまい、
  // 既存の数式や(行を先に用意して手動でメモ書きしていた場合の)入力内容を消してしまう
  // 恐れがあった。ここに列名を追加すれば、同様に「スクリプトからは触れさせたくない列」を
  // 増やせる。
